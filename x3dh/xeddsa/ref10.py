...
from xeddsa import _crypto_sign
    ...
    def empty(cls: Type[T]) -> T:
        if cls.FFI_TYPE == NotImplemented:
            raise NotImplementedError(
                "You can't use FFIType directly, but have to subclass it!"
            )
...
def fe_tobytes(fe: FieldElement) -> FieldElementBytes:
    ...
    _crypto_sign.lib.crypto_sign_ed25519_ref10_fe_tobytes(result.get(), fe.get())
...
def fe_1() -> FieldElement:
    ...
    _crypto_sign.lib.crypto_sign_ed25519_ref10_fe_1(result.get())
...
def fe_add(addend_a: FieldElement, addend_b: FieldElement) -> FieldElement:
    ...
    _crypto_sign.lib.crypto_sign_ed25519_ref10_fe_add(
        result.get(), addend_a.get(), addend_b.get()
    )
...
def fe_sub(minuend: FieldElement, subtrahend: FieldElement) -> FieldElement:
    ...
    _crypto_sign.lib.crypto_sign_ed25519_ref10_fe_sub(
        result.get(), minuend.get(), subtrahend.get()
    )
...
def fe_mul(multiplicand: FieldElement, multiplier: FieldElement) -> FieldElement:
    ...
    _crypto_sign.lib.crypto_sign_ed25519_ref10_fe_mul(
        result.get(), multiplicand.get(), multiplier.get()
    )
...
def fe_invert(fe: FieldElement) -> FieldElement:
    ...
    _crypto_sign.lib.crypto_sign_ed25519_ref10_fe_invert(result.get(), fe.get())
...
def ge_tobytes(ge: GroupElementP2) -> GroupElementP2Bytes:
    ...
    _crypto_sign.lib.crypto_sign_ed25519_ref10_ge_tobytes(result.get(), ge.get())
...
def ge_p3_tobytes(ge: GroupElementP3) -> GroupElementP3Bytes:
    ...
    _crypto_sign.lib.crypto_sign_ed25519_ref10_ge_p3_tobytes(result.get(), ge.get())
...
def ge_double_scalarmult_vartime(
    sc_a: ScalarBytes, ge: GroupElementP3, sc_b: ScalarBytes
) -> GroupElementP2:
    result = GroupElementP2.empty()

    _crypto_sign.lib.crypto_sign_ed25519_ref10_ge_double_scalarmult_vartime(
        result.get(), sc_a.get(), ge.get(), sc_b.get()
    )
...
def sc_reduce(sc: ScalarReduceBytes) -> ScalarBytes:
    _crypto_sign.lib.crypto_sign_ed25519_ref10_sc_reduce(sc.get())

    return sc.to_scalar()


def sc_muladd(
    multiplicand: ScalarBytes, multiplier: ScalarBytes, addend: ScalarBytes
) -> ScalarBytes:
    result = ScalarBytes.empty()

    _crypto_sign.lib.crypto_sign_ed25519_ref10_sc_muladd(
        result.get(), multiplicand.get(), multiplier.get(), addend.get()
    )
...
# XEdDSA additions                                                            #
###############################################################################
BASE_POINT_ORDER_MINUS_ONE = ScalarBytes.wrap(
    bytes(
        [
            0xEC,
            0xD3,
